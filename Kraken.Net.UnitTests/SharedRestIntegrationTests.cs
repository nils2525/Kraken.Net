...
namespace Kraken.Net.UnitTests
{
    ...
    internal class SharedRestIntegrationTests
    {
        ...
        private IKrakenRestClientSpotApiShared GetSpotRestClient()
        {
            var collection = new ServiceCollection();
            collection.AddKraken(x => x.Rest.OutputOriginalData = true);
            ...
            return sp.GetRequiredService<IKrakenRestClient>().SpotApi.SharedClient;
        }

        private IKrakenRestClientFuturesApiShared GetFuturesRestClient()
        {
            var collection = new ServiceCollection();
            collection.AddKraken(x => x.Rest.OutputOriginalData = true);
            collection.AddLogging(x =>
            ...
            var sp = collection.BuildServiceProvider();
            return sp.GetRequiredService<IKrakenRestClient>().FuturesApi.SharedClient;
        }
